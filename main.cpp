...
int finalBoard[8][8];//决赛用棋盘，用来表示每个格当前的状态，0表示这个格里没有东西，1表示当前格子里是ai的棋子，2表示当前格子里是玩家的棋子
QVector<QPoint> myPoint;//按顺序记录myBoard中的棋子坐标
CHESS mychess[2];//0记录AI棋子的信息，1记录玩家棋子的信息
singalOBS littleOBS[112];
mOBSTACLE myOBS[98];
...
int main(int argc, char *argv[])
{
    ...
    for(int i=0; i<7; i++)
    {
        for(int j=0; j<7; j++)
        {
            myOBS[count].obs1 = j * 7 + i;
            myOBS[count].obs2 = (j+1) * 7 + i;
            count++;
        }
    }
    for(int i=0; i<7; i++)
    {
        for(int j=0; j<7; j++)
        {
            myOBS[count].obs1 = 56 + j*7 + i;
            myOBS[count].obs2 = 56 + (j+1) * 7 + i;
            count++;
        }
    }
